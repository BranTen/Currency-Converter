...
public class GUI extends JFrame implements ActionListener {
    ...
    public GUI() {//creation of window an buttons etc
        try {
            UIManager.put("TextField.inactiveBackground", new ColorUIResource(new Color(255, 255, 255)));//-----------------added line of code
                ...
                public void keyPressed(KeyEvent e) {
                    ...
                    if (e.getKeyChar() >= '0' && e.getKeyChar() <= '9' || e.getKeyChar() == '.' ||
                        ...
                        if (e.getKeyChar() == '.' && from.getText().contains(".")) {// restricts to only one decimal point by searching if there is one already
                            ...
                            from.setEditable(false);// frevents character from being typed//----------------------------------------------------------------------------added lines of code
                            JOptionPane.showMessageDialog(frame,
                                    "only numerical values and one decimal point are allowed",
                                    "User error",
...
// to be done later
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        try {
            ...
            rate.setText("Test Rate: " + String.format("%.2f", testRate));//------------------------------------modified set text lines
